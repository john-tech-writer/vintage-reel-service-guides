...
You can easily see where any reel you encounter fits into this chart by reading the background page on that reel type, for example, the [Spinning Reels page](spinning-reels.md).

## Mid-Century Reel Types

```mermaid

flowchart TD
  A[Mid-century reels]
  A --> S[Spinning and spincast]
  A --> B[Mid century baitcasting]
  A --> F[Fly]

  S --> S1[Open face]
  S --> S2[Closed face]

  S1 --> B1[Full bail spinning]
  S1 --> B2[Finger pick up]
  S1 --> B3[Manual or roller pick up]

  S2 --> C1[Closed face spinning]
  S2 --> C2[Push button spincast]

  B --> Bait1[Typical round casting reels]

  F --> Fly1[Manual or click pawl]
  F --> Fly2[Automatic]
