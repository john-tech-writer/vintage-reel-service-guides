...
- `Video/vintage reel service guides/`
  - `audio test files/` ← e.g., for trying different render settings from Reaper
  - `cleaning-lubricating/` ← all files for this reference service video
  - `[maker]/`
    - `[model]/` ← raw mkv files from Guvcview > copied from card
      - `[slug]-overview/` ← all files for video
      - `[slug]-service guides/` ← all files for all service videos for a specific reel
